  ...
  <div>
    ...
    <v-card class="home-app-wrapper">
      <v-card-title style="color: #02786b" class="display-1">
        <span>Schedule Before Reduce Interest</span>
      </v-card-title>
    </v-card>
    <v-card>
              ...
              <template v-slot:items="props">
                <tr>
                  ...
                  <td class="text-xs-center">
                    {{
                      String(props.item.totalIntcomp).replace(
                        /(\d)(?=(?:\d{3})+(?:\.|$))/g,
                        '$1,'
                      )
                    }}
                  </td>
                  <td class="text-xs-center">{{ props.item.oldEffrate }}</td>
                  <td class="text-xs-center">
                    {{
                      String(props.item.anr).replace(
                        /(\d)(?=(?:\d{3})+(?:\.|$))/g,
                        '$1,'
                      )
                    }}
                  </td>
                  <td class="text-xs-center">{{ props.item.newEffrate }}</td>
                  <td class="text-xs-center">
                    {{
                      String(props.item.totalIntcompBf1).replace(
                        /(\d)(?=(?:\d{3})+(?:\.|$))/g,
                        '$1,'
                      )
                    }}
                  </td>
                  <td class="text-xs-center">{{ props.item.newEffrate2 }}</td>
                  <td class="text-xs-center">
                    {{
                      String(props.item.addDonw).replace(
                        /(\d)(?=(?:\d{3})+(?:\.|$))/g,
                        '$1,'
                      )
                    }}
...
export default {
  ...
  data() {
    return {
      ...
      headers: [
        ...
        { text: 'TOTAL_INTCOMP', sortable: false, align: 'center' },
        { text: 'OLD_EFFRATE', sortable: false, align: 'center' },
        { text: 'ANR', sortable: false, align: 'center' },
        { text: 'NEW_EFFRATE', sortable: false, align: 'center' },
        { text: 'TOTAL_INTCOMP_BF1', sortable: false, align: 'center' },
        { text: 'NEW_EFFRATE_2', sortable: false, align: 'center' },
        { text: 'ADDONW ', sortable: false, align: 'center' }
      ],
      RepaymentHeaders: [
        { text: 'APP_ID_C', sortable: false, align: 'center' },
  ...
  methods: {
    ...mapActions('scheduleBfReduceInterest', [
      'getReduceInterest',
      'getRepayment',
      'exportExcelFile'
    ]),
    ...
    async getData() {
      ...
      var resultRepayment = await this.getRepayment(param);
      if (result.success) {
        this.dataReduceInterest = result.data.resultData;
    ...
    exportReduceInterest() {
      ...
      var exporter;
      if (this.dataReduceInterest.length < 1) {
      ...
      var data = this.dataReduceInterest;
