<template>
  <v-container class="home-app-wrapper" grid-list-md fluid text-xs-center>
    <v-card style="width:100%">
      ...
      >Account Management By Group</v-card-title
      >
            ...
            <v-card>
              <v-card-text>
                ...
                <v-layout row wrap justify-center>
                  ...
                  <v-flex xs12 md3>
                    <v-btn
                      :disabled="showAddForm.acctPermissGroupFeature"
                      color="success"
                      class="vbutton"
                      style="margin-top: 20px;"
                      @click="handleBtnAcctPermissGroupFeature"
                    >Grant Permission Group</v-btn
                    >
                  </v-flex>
                  <v-flex xs12 md3>
                    <v-btn
    ...
    <acct-permiss-group-feature
      :show="showAddForm.acctPermissGroupFeature"
      :params="{ data: AccountInfo.accountId }"
      @shown="showAddForm.acctPermissGroupFeature = $event"
    />
  </v-container>
</template>

<script>
import AcctPermissGroupFeature from './popup_AcctPermissGroupFeature';
import { mapActions } from 'vuex';
export default {
  components: {
    AcctPermissGroupFeature
  },

  data() {
    return {
      showAddForm: {
        acctPermissGroupFeature: false
      },
      AccountInfo: {
        id: '',
        accountId: '',
      ...
      headers: [
        ...
        { text: 'User', sortable: true, value: 'accountId' },
        { text: 'Department ID', sortable: true, value: 'departmentId' },
        { text: 'Department', sortable: true, value: 'departmentName' }
      ],
  ...
  methods: {
    ...mapActions('accountmanagement_group', [
      ...
      'insUserAccount',
      'delUserAccountGroupPermiss',
      'resetPass'
    ]),

    handleBtnAcctPermissGroupFeature() {
      this.showAddForm.acctPermissGroupFeature = true;
    },
    ...
    async deleteRegisterUserAcct(chooseItem) {
      ...
      const result = await this.delUserAccountGroupPermiss(this.AccountInfo);
      if (result.success == true) {
