...
export default {
  ...
  watch: {},
  created() {
    this.minPicker = this.parseDate(
      new Date(new Date().getTime() - 1440 * 60 * 60 * 1000).toLocaleDateString(
        'en-US'
      )
    );
  },
  ...
  methods: {
    ...
    async nextPages() {
      ...
      const result = await this.getUploadFileData(param);

      if (result.data != null) {
        this.dataUploadFile = result.data;
      } else {
    ...
    async getData() {
      ...
      if (result.data != null) {
        this.dataUploadFile = result.data;
        const resultPage = await this.getTotalRows(param);
        this.totalData = resultPage.data;
        this.showPagin = true;
      } else {
        this.showWarningMsg(result.message);
      }
    ...
    async exportReport() {
      if (this.dataUploadFile.length == 0) {
      ...
      if (result.data != null) {
        ...
        if (link.download !== undefined) {
          ...
          link.target = '_blank';
          link.download =
            'Upload File Report ' + moment(today).format('DD/MM/YYYY');
          +'.csv';
          link.click();
          this.loading = false;
        }
