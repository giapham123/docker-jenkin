import DRSStaffInfoService from 'modules/staff_info/drs_staff/store/service';
...
export const getInfoAccount = async (dispatch, params) => {
  const result = await serviceAssignment.getInfoAccount(params);
  if (result.status == 200) {
    return result.data;
  }
  return null;
...
export const getInfo1 = async (dispatch, params) => {
  const result = await serviceAssignment.getInfo1(params);
  if (result.status == 200) {
    return result.data;
  }
  return null;
