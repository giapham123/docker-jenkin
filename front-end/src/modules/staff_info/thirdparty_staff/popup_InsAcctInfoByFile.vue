          ...
          <v-layout class="header white--text no_margin">
            <v-flex md12>
              <v-card-title class="white--text title">
                Import file excel for inserting information
              </v-card-title>
            </v-flex>
          </v-layout>
...
export default {
  ...
  methods: {
    ...
    ...mapActions('thirdparty_staff', ['importData']),
    ...
    async handleBtnAddClick() {
      this.disableAdd = true;
      let resultImport = await this.importData(this.listDataImp);
    ...
    downloadTemplate() {
      ...
      let exporter = new Exporter('thirdTemplate');
      ...
      exporter.addSheet(headers, this.listClear, 'Sheet0');
    ...
    handleBtnExportClick() {
      this.export_loading = true;
      let exporter = new Exporter('thirdList');
      exporter.setConfigurations({
        REPLACE_UNKNOWN_TO_EMPTY: true,
        DECORATE_BORDER: true,
        DECORATE_HEADER_COLOR: '#1f4e78'
    ...
    async importDataListDrs() {
      ...
      if (dataImport.data != null && dataImport.data.length > 0) {
        dataImport.data.forEach(element => {
          dataCheck = {};
          var strNote = '';
          if (element.accountId == '' || element.accountId == null) {
            strNote = strNote + ', Account ID null';
            impFile = false;
          }
          if (element.fullName == '' || element.fullName == null) {
            strNote = strNote + ', Full Name null';
            impFile = false;
          }
          ...
          dataCheck.fullName = element.fullName;
      ...
      this.$message.success('Import file successful!');
