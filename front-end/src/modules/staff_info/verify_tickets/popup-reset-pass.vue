<template>
  ...
  >
    ...
    <v-dialog v-model="dialogResetPass" max-width="450">
      <v-card>
        ...
        <v-card-text>
          <h5>
            Would you like <b>{{ statusResetPass }}</b> to Reset Password?
            <br />
            <b> {{ listResetPass }} </b>
          </h5>
        </v-card-text>
...
export default {
  ...
  methods: {
    ...mapActions('verifyTickets', [
      'getListResetPassRequest',
      'updVerifyResetPassResult'
    ]),
    ...
    async loadData() {
      ...
      if (this.params) {
        ...
        let result = await this.getListResetPassRequest(
    ...
    handleBtnResetPassApprove() {
      if (this.selected.length > 0) {
        ...
        this.statusResetPass = 'APPROVED';
        this.listResetPass = '';
        this.selected.forEach(element => {
          this.listResetPass += element.accountId + ' : ' + element.password;
        });
      }
    },
    ...
    async resetPass() {
      let result = await this.updVerifyResetPassResult({
        decision: this.statusResetPass,
