        ...
        <v-card-text class="body">
          <v-container grid-list-md align-content-center>
            ...
            <v-layout>
              ...
              <v-flex xs4>
                <v-textarea ref="note" v-model="decisionNote" />
              </v-flex>
            </v-layout>
            <v-layout>
              <v-flex md2></v-flex>
              <v-flex md2>
                <v-btn
                  color="#088e7d"
                  class="white--text"
                  @click="handleBtnVerifyUpdateInfo"
                  >Submit
                </v-btn>
              </v-flex>
            </v-layout>
...
export default {
  ...
  methods: {
    ...
    async handleBtnVerifyUpdateInfo() {
      if (this.selectResult.selected != null) {
        let result = await this.verifyUpdateInfoResult({
          requestId: this.params.data.requestId,
        ...
        if (result.success) {
          this.$message.success('Verify update info account success');
          this.dialog = false;
        } else {
