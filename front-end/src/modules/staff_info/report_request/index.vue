          ...
          <v-flex md12>
            <v-card>
              <v-card-text>
                <v-layout row wrap>
                  <v-flex md2>
                    <date-picker v-model="fromdate" label="From" icon="event" />
                  </v-flex>
                  <v-flex md2>
                    <date-picker v-model="todate" label="To" icon="event" />
                  </v-flex>
                  <v-flex md2>
                    <v-text-field
                      ref="requestId"
...
export default {
  ...
  created() {
    this.getDepartment();
    this.getRequestType();
    this.todate = new Date().toLocaleDateString('en-US');
    this.fromdate = new Date().toLocaleDateString('en-US');
  },

  methods: {
    ...mapActions('reportRequest', [
      'getAllDepartment',
