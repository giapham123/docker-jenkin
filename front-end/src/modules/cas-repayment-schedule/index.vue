      ...
      <v-container fluid grid-list-md>
        ...
        <v-layout>
          ...
          <v-flex xs4>
            <v-layout>
              <v-card-text style="font-weight: bold;"
              >Not_Overdue_Principal:</v-card-text
              >
          ...
          <v-flex xs4>
            <v-layout>
              <v-card-text style="font-weight: bold;"
              >Early payment penalty:</v-card-text
              >
        ...
        <v-layout>
          <v-flex xs4>
            <v-layout>
              <v-card-text style="font-weight: bold;"
              >Termination Amount:</v-card-text
              >
        ...
        <v-layout wrap>
          <v-flex>
            <v-data-table
              :headers="headers"
              :items="casRepaymentScheduleData"
            ...
            >
              <template v-slot:items="props">
                <tr>
                  ...
                  <td>
                    {{ String(props.item.instlAmt).replace(/(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g, '$1,') }}</td>
                  </td></td>
                  <td>
                    {{ String(props.item.prinComp).replace(/(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g, '$1,') }}</td>
                  </td></td>
                  <td>
                    {{ String(props.item.intComp).replace(/(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g, '$1,') }}</td>
                  </td></td>
                  <td>{{ props.item.effRate }}</td>
                  <td>
                    {{ String(props.item.balPrin).replace(/(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g, '$1,') }}</td>
                  </td></td>
...
export default {
  ...
  methods: {
    ...mapActions('casRepaymentSchedule', [
      'getDataCasRepayment',
      'exportData'
    ]),
    ...mapActions('global', [
    ...
    async getData() {
      if (
        ...
        this.agreementId.trim() == null ||
        this.agreementId.trim() == ''
      ) {
      ...
      if (result.data != null) {
        ...
        this.receivableTill = String(result.data.receivableTill).replace(
          /(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g,
          '$1,'
        );
        this.notOverdue = String(result.data.notOverdue).replace(
          /(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g,
          '$1,'
        );
        this.rsEarlyPayment = String(result.data.rsEarlyPayment).replace(
          /(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g,
          '$1,'
        );
        this.recAmount = String(result.data.recAmount).replace(
          /(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g,
          '$1,'
        );
        this.waiveOffCovid = String(result.data.waiveOffCovid).replace(
          /(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g,
          '$1,'
        );
        this.rsTerminalAmount = String(result.data.rsTerminalAmount).replace(
          /(?<!\..*)(\d)(?=(?:\d{3})+(?:\.|$))/g,
          '$1,'
        );
      } else {
